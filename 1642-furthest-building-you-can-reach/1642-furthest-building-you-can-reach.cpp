...
/////TC:O(nlogn)---performing insert operation in our priority queue (min-heap). The insert operation is done in O(log n) time and traverse through entire array once it is done in O(n) time where n is the number of buildings
//// SC:O(n) extra space in form of priority queue 
